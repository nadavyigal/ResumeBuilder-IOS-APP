...
Every time bound is day-aligned to UTC midnight, and the read is stated
"as of" that instant. Fixed bounds prevent the query window from moving between
runs; late ingestion or person merges can still change the returned data.
Compare repeated outputs as observations, not a guarantee of identical results.
...
CREDENTIALS
-----------
`AGENTIC_OS_POSTHOG_API_KEY` is read only from the environment. It is never
printed, never written to a file, and never passed on a command line. If it is
absent this script stops; it does not read local credential files.
"""
...
def load_api_key() -> str:
    ...
    if key:
        return key
    raise ContractError(
        f"{POSTHOG_KEY_ENV} is not set. "
        "This script reads credentials only from the environment; export it and re-run."
    )
...
def cohort_funnel(
    ...
    younger selections are reported separately as pending.
    """
    in_cohort = (
    ...
    def first(event: str, alias: str) -> str:
        return f"min(if(event = {lit(event)} AND {public}, timestamp, NULL)) AS {alias}"
...
SELECT
  ...
  countIf(cohort AND NOT internal AND pending)                       AS ext_pending
FROM (
  SELECT
    internal,
    cohort,
...
def build_report(args, api_key: str) -> tuple[str, str]:
    ...
    # Fixed daily bounds; late ingestion or person merges may change results.
    ...
    add(f"Read as of    : {stamp(as_of)} (day-aligned bounds; late ingestion may change results)")
    ...
    if cohort_row is not None and not (
        ...
        if min(counts['ext_selected'], counts['ext_started'], counts['ext_completed']) >= MIN_N_FOR_A_RATE:
            add(f"        activation rate : {rate(counts['ext_completed'], counts['ext_selected'])}")
        else:
            add("        activation rate : IMMATURE — counts only (at least one step below n=10)")
        add("")
    ...
    if age_hours < D7_HOURS:
        ...
        add("        VERDICT             : release age eligible; each included user also needs 168h")
    add("")
